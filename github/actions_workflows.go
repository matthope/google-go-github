...
func (s *ActionsService) CreateWorkflowDispatchEventByID(ctx context.Context, owner, repo string, workflowID int64, event CreateWorkflowDispatchEventRequest) (*WorkflowDispatchEventResponse, *Response, error) {
...
//
//meta:operation POST /repos/{owner}/{repo}/actions/workflows/{workflow_id}/dispatches
func (s *ActionsService) CreateWorkflowDispatchEventByFileName(ctx context.Context, owner, repo, workflowFileName string, event CreateWorkflowDispatchEventRequest) (*WorkflowDispatchEventResponse, *Response, error) {
	u := fmt.Sprintf("repos/%v/%v/actions/workflows/%v/dispatches", owner, repo, workflowFileName)

	return s.createWorkflowDispatchEvent(ctx, u, &event)
}

type WorkflowDispatchEventResponse struct {
	WorkflowRunID int64 `json:"workflow_run_id,omitempty"`
}

func (s *ActionsService) createWorkflowDispatchEvent(ctx context.Context, url string, event *CreateWorkflowDispatchEventRequest) (*WorkflowDispatchEventResponse, *Response, error) {
	req, err := s.client.NewRequest("POST", url, event)
	if err != nil {
		return nil, nil, err
	}

	workflowDispatchResponse := new(WorkflowDispatchEventResponse)
